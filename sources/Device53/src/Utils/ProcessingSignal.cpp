...
static int   CalculatePeriodAccurately(Channel chan);   // “очно вычисл€ет период или целое число периодов в точках сигнала.
...
static float CalculateMinRel(Channel chan);           // ¬озвращает минимальное значение относительного сигнала
static float CalculateMinSteadyRel(Channel chan);     // ¬озвращает минимальное установившеес€ значение относительного сигнала
static float CalculateMaxRel(Channel chan);           // ¬озвращает максимальное значение относительного сигнала
static float CalculateMaxSteadyRel(Channel chan);     // ¬озвращает максимальное установившеес€ значение относительного сигнала
static float CalculateAverageRel(Channel chan);       // ¬озвращает среденее значение относительного сигнала
...
static float FindIntersectionWithHorLine(Channel chan, int numIntersection, bool downToUp, uint8 yLine);  // Ќайти точку пересечени€ сигнала с горизонтальной линией, проведЄнной на уровне yLine. numItersection - пор€дковый номер пересечени€, начинаетс€ с 1. downToTop - если true, ищем пересечение сигнала со средней линией при прохождении из "-" в "+".
...
float CalculateVoltageMax(Channel chan)
{
    ...
    {
        markerHor[chan][0] = (int)max;                           // «десь не округл€ем, потому что max может быть только целым
    }
...
float CalculateVoltageMin(Channel chan)
{
    ...
    {
        markerHor[chan][0] = (int)min;                           // «десь не округл€ем, потому что min может быть только целым
    }
...
float CalculateTimeNarastaniya(Channel chan)                    // WARN «десь, возможно, нужно увеличить точность - брать не целые значени рассто€ний между отсчЄтами по времени, а рассчитывать пересечени€ линий
{
    float maxSteady = CalculateMaxSteadyRel(chan);
    float minSteady = CalculateMinSteadyRel(chan);
...
float CalculateTimeSpada(Channel chan)                          // WARN јналогично времени нарастани€
{
    float maxSteady = CalculateMaxSteadyRel(chan);
    float minSteady = CalculateMinSteadyRel(chan);
